...
// updated to Molecular Ecology 2004
// updated 10_02
...
/* double onetruncated_qnorm(double, double);
 double onetruncated_qnorm(double mean, double sd){
 double temp, temp2;
 temp2 = (mean-1.0)/sqrt(2*sd*sd);
 temp = erf(temp2);
 return 0.5+0.5*temp;
 }
 */
...
// social --> solitary (q) + solitary --> social (p)
double s_a; // a coefficient
double MAX_s_a = 1;
double MIN_s_a = 0.1; //arbitary value, but consiering the biological reality

double s_b; // a coefficient
double MAX_s_b = 1;
...
//double record_p;
//double *pointer_p = &record_p;    // settgin a pointer for the "address" of record_p
...
double p(double phenotype, double s_a){
    ...
    //*pointer_p = temp1;       // putting temp1 into a place whose "address" is "pointer_p
    return temp2;
}
...
//double record_q;
//double *pointer_q = &record_q;    // settgin a pointer for the "address" of record_q
...
double q(double phenotype, double s_b){
    ...
    //*pointer_q = temp1;       // putting temp1 into a place whose "address" is "pointer_q
    return temp1;
}
...
double cal_LL(double MRCA, double ev_rate, double eu_1, double eu_2, double s_a, double s_b){
    ...
    // setting spp i=0
    phenotype[0] = MRCA;
    if (0.5 > drand48()) spp_soc[0] = 0; // how can we decide using p and q?? 1018
    else spp_soc[0] = 1;
    ...
    for (i=1; i<N; i++) {    // note we start from i=1, not 0
        ...
        for (x=0; x<temp_total; x++) {
            ...
            if (spp_soc[i] == 1) {      // in the case that spp_i is social, start BM
            ...
            else if (spp_soc[i] == 0) { // in the case that a given spp is solitary, GS also moving as solitary by BM
    ...
    if (temp_sol == 4 && temp_soc == 6){
        for (i=0; i<N; i++) {
            if (tip[i]==1 && social[i]==1){
                ...
                ;
                // deleted - log(onetruncated_qnorm(phenotype[i], spp_sd[i])) to make it back to normal dist. 1018
                // deleted log((1/(sqrt(2*PI)*spp_sd[i]))) because this is a constant irrespective to simulation results
                // this means that "lik" is not a "absolute" but "relative" likelihood by deleting this.
            }
        }
